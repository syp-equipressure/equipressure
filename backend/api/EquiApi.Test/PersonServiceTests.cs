...
using PersonService = EquiApi.Persistence.Model.Person;
...
public sealed class PersonServiceTests
{
    ...
    private readonly ILogger<Core.Services.PersonService> _loggerMock = Substitute.For<ILogger<Core.Services.PersonService>>();
    private readonly Core.Services.PersonService _sut;

    public PersonServiceTests()
    {
        _sut = new Core.Services.PersonService(_uowMock, _dateTimeProviderMock, _loggerMock);

        _dateTimeProviderMock.GetCurrentDate().Returns(new LocalDate(2026, 1, 1));
    }

    private static PersonService CreatePerson(int id = 1) => new()
    {
        Id = id,
        FirstName = "Max",
        LastName = "Mustermann",
    ...
    private static HorseService CreateHorse(int id = 1) => new()
    {
        Id = id,
        Name = "Hugo",
    ...
    public async Task GetFavouritesAsync_ListEmpty_ReturnsNone()
    {
        ...
        _uowMock.PersonRepository.GetFavouritesAsync(personId).Returns(new List<PersonService>());
    ...
    public async Task GetFavouritesAsync_ListNotEmpty_ReturnsSuccess()
    {
        const int personId = 1;
        _uowMock.PersonRepository.PersonExistsAsync(personId).Returns(true);
        _uowMock.PersonRepository.GetFavouritesAsync(personId).Returns(new List<PersonService> { CreatePerson() });
    ...
    public async Task GetContactsAsync_ListEmpty_ReturnsNone()
    {
        const int personId = 1;
        _uowMock.PersonRepository.PersonExistsAsync(personId).Returns(true);
        _uowMock.PersonRepository.GetContactsAsync(personId).Returns(new List<PersonService>());
    ...
    public async Task GetContactsAsync_ListNotEmpty_ReturnsSuccess()
    {
        const int personId = 1;
        _uowMock.PersonRepository.PersonExistsAsync(personId).Returns(true);
        _uowMock.PersonRepository.GetContactsAsync(personId).Returns(new List<PersonService> { CreatePerson() });
    ...
    public async Task GetOwnedHorsesAsync_ListEmpty_ReturnsNone()
    {
        const int personId = 1;
        _uowMock.PersonRepository.PersonExistsAsync(personId).Returns(true);
        _uowMock.PersonRepository.GetOwnedHorsesAsync(personId).Returns(new List<HorseService>());
    ...
    public async Task GetOwnedHorsesAsync_ListNotEmpty_ReturnsSuccess()
    {
        const int personId = 1;
        _uowMock.PersonRepository.PersonExistsAsync(personId).Returns(true);
        _uowMock.PersonRepository.GetOwnedHorsesAsync(personId).Returns(new List<HorseService> { CreateHorse() });
    ...
    public async Task AddPerson_ValidData_SavesToDatabaseAndReturnsSuccess()
    {
        ...
        _uowMock.PersonRepository.Received(1).AddPerson(Arg.Any<PersonService>());
    ...
    {
        const int personId = 1;
        _uowMock.PersonRepository.GetPersonByIdAsync(personId).Returns((PersonService?)null);
